...
const EN = {
  ...
  billing: {
    ...
    body: (kind: string, scope: string, d: string, amount: string) => ({
      ...
      trial_ended: `The free trial for <strong>${scope}</strong> has ended. Renew from the Billing page — choose your cycle and pay <strong>${amount}</strong>. You keep full access until <strong>${d}</strong>; after that the account locks until you renew.`,
      renewal_7: `The current period for <strong>${scope}</strong> ends on <strong>${d}</strong>. Renew from the Billing page (<strong>${amount}</strong>) to keep everything running — unrenewed at period end, the account locks until you do.`,
      ...
      grace: `The trial for <strong>${scope}</strong> ended without a subscription. Everything still works until <strong>${d}</strong>; after that the account locks until you subscribe (<strong>${amount}</strong>).`,
      locked: `The account for <strong>${scope}</strong> is locked: pages are read-only and residents cannot sign in. Renewing from the Billing page (<strong>${amount}</strong>) unlocks it immediately.`,
...
const AR: Dict = {
  ...
  billing: {
    ...
    body: (kind: string, scope: string, d: string, amount: string) => ({
      ...
      trial_ended: `انتهت التجربة المجانية لـ<strong>${scope}</strong>. جدّد من صفحة الفوترة — اختر الدورة وادفع <strong>${amount}</strong>. تحتفظ بالوصول الكامل حتى <strong>${d}</strong>؛ بعدها يُقفل الحساب حتى التجديد.`,
      renewal_7: `تنتهي الفترة الحالية لـ<strong>${scope}</strong> في <strong>${d}</strong>. جدّد من صفحة الفوترة (<strong>${amount}</strong>) ليبقى كل شيء شغالاً — إن لم تجدّد قبل نهاية الفترة يُقفل الحساب.`,
      ...
      grace: `انتهت تجربة <strong>${scope}</strong> دون اشتراك. كل شيء يعمل حتى <strong>${d}</strong>؛ بعدها يُقفل الحساب حتى تشترك (<strong>${amount}</strong>).`,
      locked: `حساب <strong>${scope}</strong> مقفل: الصفحات للقراءة فقط ولا يستطيع السكان الدخول. التجديد من صفحة الفوترة (<strong>${amount}</strong>) يفتحه فوراً.`,
...
const FR: Dict = {
  ...
  billing: {
    subj: (kind: string, scope: string) => ({
      ...
      renewal_7: `C’est le moment de renouveler ${scope}`,
    ...
    body: (kind: string, scope: string, d: string, amount: string) => ({
      ...
      trial_ended: `L’essai gratuit pour <strong>${scope}</strong> est terminé. Renouvelez depuis la page Facturation — choisissez votre cycle et payez <strong>${amount}</strong>. Vous gardez l’accès complet jusqu’au <strong>${d}</strong> ; ensuite le compte se verrouille jusqu’au renouvellement.`,
      renewal_7: `La période en cours pour <strong>${scope}</strong> se termine le <strong>${d}</strong>. Renouvelez depuis la page Facturation (<strong>${amount}</strong>) — sans renouvellement à l’échéance, le compte se verrouille.`,
      ...
      grace: `L’essai pour <strong>${scope}</strong> s’est terminé sans abonnement. Tout fonctionne jusqu’au <strong>${d}</strong> ; ensuite le compte se verrouille jusqu’à l’abonnement (<strong>${amount}</strong>).`,
      locked: `Le compte de <strong>${scope}</strong> est verrouillé : lecture seule, et les résidents ne peuvent plus se connecter. Renouveler depuis la page Facturation (<strong>${amount}</strong>) le déverrouille immédiatement.`,
