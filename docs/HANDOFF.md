...
| 0126 | `0126_beta_code_scope.sql` | **Scoped beta codes** (Whish onboarding review). `beta_access_codes.scope`: 'full' (testers, default) or 'demo' (marketing site + demo only — Register bounces to /demo, so the pricing model stays private pre-launch). New RPC `beta_code_scope(code)` returns the scope; the gate stores it (`abniyah_beta_scope` localStorage) and `betaScope()` in lib/demo.ts reads it app-wide. `verify_beta_code` unchanged for the old bundle — **deploy the client BEFORE creating any demo-scoped code**, or the old bundle accepts it as full. Companion (same commit): `DEMO_HIDDEN_ROUTES` hides Licenses/Import/Dues/Collect from the demo personas (sidebar + URL guard). |
| 0127 | `0127_email_exists.sql` | **Email existence check** for forgot password. `email_exists(p_email)` (SECURITY DEFINER over auth.users, boolean only, anon-callable: the flow runs pre-auth). Login shows "No account found" instead of silently sending to nowhere. Deliberate trade-off: accepts email enumeration for clearer UX (Jey's call, Aug 2026); Supabase rate-limits per IP. Client falls through to the old silent send if the RPC is missing. |
| 0128 | `0128_fix_repost_expense_scope.sql` | **SECURITY (audit C1, critical).** `repost_expense()` trusted client `unit_id`/`building_id` and could write charges onto any unit in any building, silently. Now rejects any row outside the expense's building/compound and derives `building_id` from the unit (mirrors the 0093 guard). |
| 0129 | `0129_fix_settle_superseded_intent.sql` | **SECURITY/MONEY (audit H5, high).** `settle_payment_intent` settled `expired`/`failed` intents (Whish link to a superseded intent stays live → wrong-plan/double-charge). Now rejects any non-`pending` intent after the idempotent `paid` fast-path. |
| 0130 | `0130_deactivation_enforced.sql` | **SECURITY (audit H3, high).** `user_can_unlocked` never checked `profiles.status`, so a deactivated manager kept API access. Now returns FALSE for any non-platform caller whose profile is not `active`. (Resident membership policies + session revocation on deactivation are follow-ups.) |
| 0131 | `0131_fix_block_injection.sql` | **SECURITY (audit H1, high).** `buildings_insert_self_service` never checked `compound_id`, so any active user could attach a block to another tenant's compound and (via auto-grant) admin it. New `user_manages_compound()` helper; buildings/compounds insert policies + `create_building()` now require management of the parent. Standalone create unchanged. |
| 0132 | `0132_user_manages_compound_anon_gap.sql` | Hygiene: `REVOKE ... FROM PUBLIC, anon` on the `user_manages_compound()` helper added in 0131 (was anon-callable but harmless — returns false for a null caller). Matches the 0111/0119/0122 anon-gap standard. |
| 0133 | `0133_mfa_enforced.sql` | **SECURITY (audit H2, high).** 2FA was enforced only in React. New `mfa_satisfied()` (aal2 OR no verified factor) gates `user_can_unlocked`, so a 2FA user's password-only (aal1) session now holds no capability server-side. Users WITHOUT 2FA are unaffected. Rollback SQL in the migration footer; SQL Editor (service_role) bypasses RLS if you lock yourself out. |
| 0134 | `0134_fix_renewal_intent_staleness.sql` | **MONEY (audit M2, medium).** schedule/cancel/request licence-count changes now expire any pending PERIOD payment_intent, so a stale renewal Whish link can't settle a wrong (low) price. Composes with 0129's non-pending guard. |
| 0135 | `0135_storage_upload_limits.sql` | **HARDENING (audit M6, medium).** Bucket-level `file_size_limit` + `allowed_mime_types` on attachments (15MB; images/PDF/office, excludes html/svg) and avatars (5MB; images). Server-enforced regardless of client. |
| 0136 | `0136_issue_budget_unit_guard.sql` | **HARDENING (audit L2, low).** `issue_budget()` now rejects a dues row whose unit doesn't belong to its building (was: building scope checked, unit→building not). Same class as C1. |
| 0137 | `0137_audit_log.sql` | **Append-only audit trail.** `audit_log` table + generic `audit_capture()` triggers on privilege/structural tables (grants, memberships, buildings, compounds, units, orgs, subscriptions, profiles-security-cols) full I/U/D, and financial ledgers (charges, payments, adjustments, dues, invoices, budgets) U/D only. Captures actor + aal + old/new JSON. Tamper-evident: platform-admin SELECT, no update/delete policy. Non-blocking (warns, never fails a write). |
| 0138 | `0138_soft_delete.sql` | **Reversible deletes.** `deleted_at`/`deleted_by` on buildings/compounds/units/organizations. A BEFORE DELETE trigger converts the first delete into a soft-delete (cancels the cascade); deleting an already-trashed row purges for real. RESTRICTIVE SELECT policy hides trashed rows (no client change). `list_trash()` / `restore_entity()` RPCs power a recycle bin; `purge_soft_deleted()` hard-deletes after 30 days via pg_cron. Existing `.delete()` calls are now reversible with no code change. |
| 0139 | `0139_audit_expenses.sql` | Follow-up to 0137: adds `expenses` to the audit trail (full I/U/D) — it was missed. Booking/editing/deleting an expense now appears in the Activity log. (charges/payments stay UPDATE/DELETE-only; bulk INSERTs excluded by design.) |
...
| 0148 | `0148_billing_rpc_guard_flag.sql` | **Cancel-subscription bug (found live by Jey, 26 Aug).** A customer's own Cancel/Resume/Start subscription RPCs write `cancelled_at`, which sits on `subscriptions_column_guard`'s forbidden list - the trigger judges the CALLER, so every non-platform-admin got "Billing fields are managed by the Abniyah team". Fix = the 0108 flag pattern: the three sanctioned RPCs set transaction-local `abniyah.billing_rpc` after their auth check and the guard honors it; direct table writes stay blocked. Error copy de-em-dashed. |
| 0149 | `0149_events_guard_cancel_regression.sql` | **Second half of the cancel bug.** 0118's rebuild of `subscription_events_guard` silently dropped the 0114 self-service events (`cancel_requested`, `cancel_withdrawn`, `invoice_issued`, `auto_renew_*`) from its allowlist, so after 0148 the cancel RPC died on its event insert. Guard now honors the 0148 `abniyah.billing_rpc` flag (actor_id still stamped) and the allowlist is restored to the 0114 ∪ 0118 union. Run together with 0148. |
| 0150 | `0150_seat_based_licensing.sql` | **LICENSING MODEL CHANGE (Jey, 26 Aug): seats, not unit counts.** Units are free structure; assigning a licence SEAT is the wall (now DB-enforced - it was client-only) and renewal money clamps to ASSIGNED seats, never raw units. Unit-creation cap dropped; `subscription_unit_count` excludes trashed units (the 76-vs-67 import bug); trashing a unit releases its seat so a restore always returns UNLICENSED; licence floor + scheduled reductions count assigned only. Companion client (same day): unit import upserts on (resolved building, label) with an Update/Skip choice, auto-assigns seats while the pool lasts, then creates unlicensed with a note. ⚠️ Money-adjacent - exercise the post-run checks on Bahloul. Phase 2 open question: what an UNLICENSED unit functionally loses (decide by Whish launch). |
| 0151 | `0151_invite_accept_activation.sql` | **Invite flow: pending until accepted (Jey, 26 Aug).** invite-user now creates profiles `pending`; `activate_invited_account()` flips them to active on the invitee's FIRST sign-in, but only with invite evidence (home building / grant / membership) - strays stay pending, so the stranger gate survives. The redundant "resident approved" email is removed from dynamic-action (it also fired with building '—' on imports). Invitations tab lists invited accounts awaiting sign-in. Companion: unit import gains optional Owner/Tenant Name columns (fixes email-as-name; upgrades old email-named profiles) and stamps the invitee's home building. **Redeploy invite-user AND dynamic-action.** |
| 0152 | `0152_onboarding_blocks.sql` | **Final import design (Jey, 26 Aug).** A compound's blocks are named IN the register wizard (`complete_admin_onboarding` gains `p_blocks`, default NULL - old callers unchanged), so compounds are born fully shaped. Companion client: building/compound admins' Bulk Import shrinks to **Structure** (units + people in one sheet: label, floor, share, owner/tenant name + email + phone with country code) **+ Expenses & Balances**; Users/Buildings imports stay org/platform-only. Structure import VALIDATES strictly: block names must match existing blocks (auto-create from 26-Aug removed - registration owns structure now), emails/phones checked (`+` country code required), all-digit names rejected. |
| 0153 | `0153_fund_currency_scope.sql` | **Cash split by currency + per-block fund scope (Jey+Ahmad, 27 Aug).** fund_position() now also returns the PHYSICAL drawers - cash_usd (sum of USD parts) and cash_lbp (raw LBP, never re-rated) - while the book stays canonical USD. compounds.fund_scope: 'compound' (one box, default, 0106 behavior) or 'block' (funds rows per building, no compound widening, compound-wide money must name the paying block via expenses.paid_from_building_id / fund_entries.building_id; unnamed rows surface as `unattributed`). funds gains opening_balance_lbp + opening_lbp_rate (frozen-rate pair); adjustments gains the LBP pair (refunds in lira). Sealed setter set_fund_scope(); switching is a physical recount - re-enter openings after. Client: Finance fund card shows both drawers + a Per compound / Per block tag + per-block breakdown table; Dashboard hero same tag + drawer subline. |
| 0154 | `0154_lost_and_found.sql` | **Lost & Found (Jey, 28 Aug).** lost_items per block; any member or manager posts (photo from the camera), every member+admin gets an in-app 'lost_found_new'; claiming is the sealed claim_lost_item() RPC (first claim wins, residents can flip nothing else); claim notifies poster+admins; managers mark returned/delete and see WHO claimed (residents only see 'claimed'). Page /lost-found. |
| 0155 | `0155_voting.sql` | **Voting (Jey, 28 Aug).** New capability poll.manage (admin roles; role_has_cap restated from 0110 - keep permissions.ts in sync, done same commit). polls (building XOR compound) with the ballot rules: eligibility all_residents/owners_only/one_per_unit, weighting per_person/by_share (share forces per-unit), single/multiple choice, secret/open ballot, allow_abstain, quorum_pct, pass_threshold_pct, results live/after_close. poll_votes has NO write policies - cast_vote() (revote till close, per-unit ballots replace, owner outranks tenant) and poll_results() (aggregates; hides per-option counts on after_close polls while open, turnout always visible) are the only doors; close_poll() for managers. 'poll_opened'/'poll_closed' notifications to residents. Page /voting. |
| 0156 | `0156_poll_defaults.sql` | **Voting rules as building policy (Jey, 28 Aug).** poll_defaults: one row per building/compound holding the ballot rules; set once under 'Voting rules' on /voting, every new vote snapshots them at creation (0155 poll columns untouched - editing defaults never rewrites a live ballot). Read: everyone in scope; write: poll.manage. New Vote modal slims to scope/question/options/closing time + a read-only rules summary. |
| 0157 | `0157_money_hardening.sql` | **Finance-review fixes (28 Aug, two-auditor pass over 0153-0156).** SQL: LBP pair CHECKs demand rate>0 (rate-0 row counted the same money in BOTH drawers); expenses.paid_from_building_id trigger-guarded to the expense's own compound; poll_results eligible weight floors shares like cast_vote (no >100% turnout). Client (same commit): Balances import STRICT-parses money (1.234,56 / 12,5 / trailing-minus become invalid rows instead of importing wrong magnitudes) and writes units.opening_balance instead of charge/payment rows (cash/Collected/Billed no longer inflated by history; zero rows skipped to protect existing openings; re-import replaces - batch undo does not apply to balance imports); Reports/PDFs learn 0153 (fund_scope, block funds, LBP openings - the Fund PDF was $-wrong on any LBP opening or block-mode compound); Finance aggregate honors each fund's own opening_date via shared openingsOf() (SQL op-CTE parity); client fundPosition defaults its cut to today and drops units created after the as-of (SQL parity); currency r2 rounds half away from zero like NUMERIC; Dashboard as-of dates formatted locally (UTC+ dropped month-end). +7 vitest cases pin drawers/rounding/as-of gates. KNOWN+accepted: cash vs cash_usd+rated-LBP agree only to rounding cents (never assert equality); fund_position().unattributed is per-compound, never sum across blocks; Dashboard block-filter has no Finance-card counterpart. |
| 0158 | `0158_special_charges.sql` | **Special charges first-class + NETTED asks (Jey's QA, 29 Aug).** special_charges table (label/total/method/party/voided) with charges.special_charge_id + payment_requests.special_charge_id; sealed create_special_charge() (client sends {unit_id,amount} only - block/party/tenant derived server-side, 0128 discipline; optionally issues the request atomically) and void_special_charge() (voids its charges + cancels its request lines). THE NETTING RULE, applied to special-charge requests AND request_payment_for_expense (restated from 0091): ask = LEAST(this charge's slice, the party's outstanding) - old arrears never dragged into an emergency ask, credit absorbs first, credit-rich units not chased (fixes both QA findings with one rule). Client: Expenses tab lists special charges with Void; special-charge modal gains a due-days select; metered expense types removed from the expense category picker (they post via Metering); dynamic-action: new-charge email/WhatsApp OFF (a charge is a ledger line, not an ask - requests/prepaid asks keep their channels; in-app bell unchanged) - NEEDS REDEPLOY. |
| 0159 | `0159_unit_delete_guard.sql` | **A unit leaves only with a settled book (Jey's QA, 29 Aug).** BEFORE DELETE guard on units, named aa_* so it fires BEFORE soft_delete_trg (0138 order-by-name): trashing requires balance EXACTLY 0 (error names the amount; balance math inlined from 0043 so the auth-less purge cron can run it); a PURGE of a unit with any charges/payments/adjustments is cancelled SILENTLY (RETURN NULL) - financial history is never cascade-deleted, the unit stays archived in the trash forever, purge_soft_deleted() skips it cleanly. deleted_unit_labels(ids) RPC (finance.view-gated) names trashed units; Finance's unitDisplay tags their rows 'Label · deleted unit' instead of a dash. Client same commit: 'Where in the Nizam' field removed from the unit form (column stays), share-weight label drops the Arabic gloss. |
| 0160 | `0160_special_charge_edit.sql` | **Special charges editable + every change notifies (Jey's QA r3, 29 Aug).** update_special_charge(): edit = replace, audited (old rows voided 'Special charge edited', new rows notify_suppressed - no bell storm), old ask cancelled, optional fresh NETTED request; residents ALWAYS get one in-app 'Special charge updated' notice, and with the request ticked the request notification goes out on every channel. void_special_charge() restated + 'Special charge removed' notice (captured before voiding). request_payment_for_expense() restated + self-replacing (cancels its own expense's live lines first - re-requesting never stacks asks). Client: request checkbox defaults OFF, days field stacks under it, the list is a real expenses-style table with Edit + Void icons, and editing an extraordinary expense auto-re-issues its netted ask. |
| 0161 | `0161_expense_void_contributions.sql` | **Expenses join the void discipline; 'special charges' become CONTRIBUTIONS (QA r4, 29 Aug).** expenses gain voided_at/by/reason + sealed void_expense() (cascades to its live charges, cancels its extraordinary ask, one in-app 'Charges removed' notice; metered refuse; hard delete REMOVED from the client - voidTarget/confirmVoid route 'expenses' through the RPC). fund_position restated with voided_at IS NULL on exo/unrec/unattr; client twin adds live() on expenses (one place, all callers). The three contribution RPCs restated with 'Contribution' wording in errors + notification titles (tables keep special_charges names). UI: Add contribution button + the contributions table MOVED to the Adjustments tab (adjustments-consistent styling: opacity-45, voidedBadge, line-through, slim action cell), rows clickable to a detail modal (Edit/Void inside) matching expenses/payments row-click parity; voided expenses show the same styling and refuse editing. |
| 0162 | `0162_metering_v2.sql` | **Metering v2 (Jey+Ahmad+Claude, 29 Aug): two models, typed purchase pull, server-side finalize, stock in the fund.** meter_settings per scope+type: model 'mbm' (bills the window's purchases; losses inherently grossed) or 'wa' (moving-average: rate=(opening value+purchases)/(opening qty+bought qty); closing value chains forward; dual billed/spot rates frozen per cycle). AHMAD'S RULE: purchases pulled ONLY from settings.purchase_expense_type_id + fund-paid + expenses.qty NOT NULL + window (qty is also the legacy/new marker - old cycle-posted expenses can never double-pull). finalize_meter_cycle() is the SOURCE OF TRUTH (client computeMeterCycle is its preview twin, vitest-pinned): posts CHARGES ONLY (no expense - kills the old cash-drift where posted expense != cash spent), guards impossible readings/stock-rose-without-invoice/meters-exceed-consumed, LOSSES_ALARM|pct above settings threshold (retry with p_confirm_losses). Switching rules: MbM->WA opening value FORCED 0 (blended rate returns residents' prepaid tank); WA->MbM gated on stock value ~0; only the LATEST final cycle re-derivable/deletable; pulled purchases void-LOCKED (void_expense restated). fund_position gains stock_on_hand (latest WA closing value; reserve formula UNCHANGED - stock is a separate line on the fund card + hero stat). Client: model chooser cards, settings modal, read-only pulled-invoice list on the cycle, expense form gains a Quantity field on purchase-bound types. |
| 0162b | *(same file, re-run it)* | **Metering v2 review hardening (code review + security review, 30 Aug).** The reviews caught: (1) `charges.meter_cycle_id` was never created (plpgsql compiles lazily - every finalize would have died at runtime) - the ALTER + index now open 0162; (2) chain/overlap/back-dating collapsed into ONE status-independent guard: finalize refuses if any OTHER final cycle of the type ends inside or after the window (a client flipping status to draft can no longer re-derive history, and the same purchase can never be pulled by two windows); (3) legacy v1 expense-backed cycles are read-only under v2 (re-deriving one would double-bill); delete guard keys on what was POSTED, not on status; (4) pulled purchases are frozen against EDITS too - new BEFORE UPDATE trigger `expenses_purchase_lock_trg` walls money/date/type/qty changes (repost included); the void lock also now applies only to fund-paid purchases; (5) finalize RETURNS the posted total (client toasts the server number, not the preview); (6) share-weight-0 zeroes only the common term (was: silently dropped every charge); readings of since-deleted units no longer price the pool; a WA free-tank run-down (pool 0) may finalize to advance the chain. Client: purchase Quantity now visible/persisted across the block<->compound boundary (Finance meter scope includes the parent compound / the blocks; edit saves qty); failed finalize keeps its draft as the editing target (no orphan-draft pileup); cycle prefill seeds from the latest FINAL; DST-safe UTC date proposal; preview prev-cycle honors the period like SQL; non-latest finals + legacy cycles read-only in the UI. |
| 0163 | `0163_user_lookup.sql` | **Import owner-link bug (QA 30 Aug): existing accounts were never linked.** invite-user called `admin.auth.admin.getUserByEmail()` which DOES NOT EXIST in supabase-js v2 - the call threw, the import swallowed the error, and the unit was created ownerless while the progress said "done". Fix: `auth_user_id_by_email(email)` SECURITY DEFINER over auth.users, EXECUTE granted to service_role ONLY (clients must never enumerate accounts); invite-user import-mode uses it. **Requires invite-user redeploy.** Client hardening in the same commit: linkParty now RETURNS a failure note (invite error or membership-insert error) and the progress row goes red with the reason instead of lying "done"; owner links BEFORE tenant (the old Promise.all raced the 0063 tenant-requires-owner trigger against itself). |
| 0164 | `0164_no_em_dash_notifications.sql` | **No em dashes in notifications (house style, 30 Aug).** Rewrites the LIVE definitions of every public function that INSERTs INTO notifications (plus my_membership_invites) via pg_get_functiondef + replace + EXECUTE - no verbatim restatement of 14 migrations, no drift risk, OIDs/trigger bindings kept; their RAISE messages are de-dashed too; idempotent (re-run finds 0). dynamic-action emails/pushes/WA params swept in the same commit (**redeploy needed**) + the last en.json straggler. Meta-frozen WhatsApp TEMPLATE bodies are out of our hands; only our params changed. |
| 0165 | `0165_inspection_chains.sql` | **Inspection categories + self-perpetuating chain (Jey, 30 Aug).** `inspection_categories` per scope (user-created, RLS mirrors inspections); inspections gain category_id + spawned_from + status 'due'. AFTER trigger `inspections_chain`: completing a record with a next-due date AUTO-CREATES the next inspection as a 'due' row on that date (re-saving moves the same successor - no duplicates; clearing the date deletes a still-pending one); existing scheduled rows backfilled. `get_due_inspections` now reads 'due' rows (same signature - **send-reminders needs NO redeploy**) and returns the user category name. Page: grouped by category, amber Due / red Overdue badges, one-click Record flow on due rows, Categories manager modal. Legacy enum rows group under their old labels. Jad updated - **help-chat redeploy**. |
| 0166 | `0166_amenity_details.sql` | **Amenity identity fields (QA, 30 Aug).** brand / serial_no / quantity (>0) / kind_other on amenities, all optional. The form shows Brand, Quantity, Serial # and - when kind = Other - a "What is it?" field; the kind badge then shows the typed name instead of "Other" (list + detail). Brand, xN and serial ride the meta lines. |
| 0167 | `0167_issue_resolver.sql` | **Issue resolver stamp (31 Aug).** resolved_by (audit) + resolved_by_name (display snapshot, 0123 inspector pattern) set when status flips to resolved, cleared on reopen; card reads "Resolved on {date} by {name}" (falls back to date-only for pre-0167 resolves). |
| 0168 | `0168_vote_by_email.sql` | **Voting email+push with ONE-CLICK ballots (31 Aug).** polls INSERT -> email+push to everyone in scope (except the creator): single-choice polls carry a signed link PER OPTION PER RECIPIENT (HMAC, key = service role key); clicking lands on the scanner-safe confirm page of the NEW `vote-click` edge function (GET never casts - mail scanners follow links; POST verifies the HMAC and votes via `cast_vote_as`, a service-role-only wrapper that impersonates the link's user INTO cast_vote so every 0155/0156 rule applies verbatim - revote replaces, so the same email works until close). polls UPDATE open->closed -> results email. emailToUserIds builders now run per member (personal links). **Needs: run 0168, deploy NEW function `vote-click` with JWT verification OFF, redeploy dynamic-action, NEW Database Webhook on polls INSERT+UPDATE.** |
| 0169 | `0169_poll_ballots.sql` | **Roll call: who voted what (31 Aug).** `poll_ballots(poll)` SECURITY DEFINER returns every eligible voter with status 'voted' (+ chosen labels), 'abstained' or 'pending'. Privacy mirrors the poll_votes RLS EXACTLY: a SECRET ballot returns 0 rows to everyone (managers and platform admin included, open or closed - closing never reveals identities); an OPEN ballot returns rows only to poll managers; residents always get 0 rows. The eligible set mirrors poll_results() so roll call and turnout can never disagree (one_per_unit -> units with a live membership, expected voter = owner else tenant; all_residents/owners_only -> people). UI: a 'Who voted what' toggle on the poll card for managers of non-anonymous polls, with a voted/abstained/pending summary line. |
| 0170 | `0170_delete_own_account.sql` | **Self-service account deletion (App Store req. 5.1.1v, 3 Sep).** can_delete_own_account() lists blockers (platform owner protected; management grants must go first; every linked unit balance must be exactly 0 - owed or credit); delete_own_account() deletes auth.users -> cascades profile/memberships/grants/device tokens; unit-anchored financial history survives with authorship columns going NULL (0026 SET NULL work). Settings gains a rose Danger Zone card: blockers explained in plain language, or type-your-email confirm -> delete -> signed out. Three languages. |
| - | *(device-QA sprint, 3 Sep - client + edge, no migration)* | **The first real-iPhone QA pass, ~15 fixes in one day.** iOS: Mac .env.local still pointed at SEOUL - the binary baked the old DB (fixed on the Mac; Frankfurt now verified on device); pull-to-refresh re-prompted Face ID (unlock now remembered in sessionStorage for the app run); stale service workers broke plain refreshes (cleanupOutdatedCaches + skipWaiting/clientsClaim). Mobile layout: app shell clips horizontal overflow (no more sideways page pan), meta-line bullets are ::before marks that wrap WITH their item across the six list pages, row right-side blocks wrap under titles, money-modal grids collapse to one column on phones. Push: APNs key normalizer tolerates literal-\n pastes (the Frankfurt secret migration corrupted the .p8 - re-pasted); pushes carry a `route` and the app deep-links on tap, cold start included (client listener in push.ts, binds at module load). Voting: ballot emails include the CREATOR (the email IS the ballot); options pulled by `position` after a wait-for-options retry. Book tab: owner sub-row shows the owner's name, and owner-only units carry it on the main row (BookRow.ownerName via tenancyHelpers). Also: add-expense defaults to Common Expenses; Issues filter defaults to Open; Lost & Found gained an Unclaimed-default filter, signed photo URLs, reported-by, detail modal, and email+push on new items. |
| - | *(client + vote-click, no migration)* | **One-click ballots move to our own domain (31 Aug).** Supabase rewrites edge-function responses on *.supabase.co to `Content-Type: text/plain` + a sandbox CSP (anti-phishing), so vote-click could never render its confirm page - browsers showed the source and offered a .txt download. vote-click is now a JSON API (GET = info, never casts; POST = cast) with CORS, and the confirm UI is a PUBLIC app route `/vote` (VoteConfirm.tsx, no login needed). Email links point at `APP_URL/vote?...`; the HMAC and cast_vote_as path are unchanged. |
| - | *(dynamic-action, no migration)* | **Lost & found email+push (31 Aug).** New lost_items INSERT case in dynamic-action: email + push to every resident of the building except the poster, three languages, building name as sender; bell trigger (0154) unchanged - one alert per channel. **Needs: dynamic-action redeploy + a NEW Database Webhook on lost_items INSERT pointing at dynamic-action (same config as the existing webhooks).** Claim events stay bell-only. |
| - | *(client only, no migration)* | **Map pin-picker (30 Aug).** "Pin on map" button beside the Google Maps Link field (create + edit building): Leaflet + OpenStreetMap modal (free, keyless, lazy-loaded chunk), tap-or-drag pin, default center Beirut, saved as a plain `google.com/maps?q=lat,lng` link - the residents' link and the embed keep working unchanged, and buildEmbedUrl now prefers the pinned coords over the typed address. New `src/components/MapPicker.tsx` (MapsLinkInput + coordsFromMapsUrl). OSM row added to DEPENDENCIES.md. |
| - | *(client + help-chat, no migration)* | **Purchase-type separation (Ahmad, 30 Aug).** The metering settings no longer default the purchase type to the metered type itself: new setups must choose it explicitly, other metered types are excluded from the list (cross-feeding), and a one-click button creates a dedicated "{type} purchases" expense type and selects it. Existing settings keep their saved value. Jad step 3 updated - **help-chat redeploy needed**. |
...
- **Payment-request label is English-prefixed in SQL** ("Extraordinary: …",
  0091 `request_payment_for_expense`), so an Arabic reader's WhatsApp/email
  gets a mixed-language description line. Localise before real buildings:
  store a kind + the bare description, render the prefix per language.
  (The WhatsApp template itself is DONE: `abniyah_payment_request` en+ar
  approved and verified live 25 Sep 2026.)
