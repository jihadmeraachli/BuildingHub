...
const statusColor: Record<InspectionStatus, 'green' | 'red' | 'yellow' | 'slate'> = {
  passed: 'green', failed: 'red', action_required: 'yellow', pending: 'slate',
};

type Form = {
  category: InspectionCategory; title: string; inspector: string;
...
const newForm = (): Form => ({
  ...
  status: 'pending', outcome: '', next_due_date: '',
});
...
export default function Inspections() {
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from('inspections').select('*').eq('building_id', buildingId).order('inspection_date', { ascending: false });
  ...
  function openEdit(r: Inspection) {
    ...
    setForm({ category: r.category, title: r.title, inspector: r.inspector ?? '', inspection_date: r.inspection_date, status: r.status, outcome: r.outcome ?? '', next_due_date: r.next_due_date ?? '' });
    setOpen(true);
  }
  ...
  async function save() {
    ...
    setSaving(true);
    const attachment_url = file ? await uploadFile('attachments', `${buildingId}/inspections`, file) : null;
    ...
    const { error } = editId
      ...
      : await supabase.from('inspections').insert({ ...base, building_id: buildingId, created_by: profile?.id });
    setSaving(false);
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {canManage && buildingId && <Button onClick={openNew}><Plus size={16} /> {t('inspections.add')}</Button>}
        </div>
      </div>
