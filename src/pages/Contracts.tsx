...
const newForm = (): Form => ({
  ...
  start_date: '', end_date: '', amount: '', billing_cycle: 'monthly', notes: '',
});
...
export default function Contracts() {
  ...
  const canManage = isPlatformAdmin || can('building.manage', buildingId) || legacyManager;

  useEffect(() => { if (!buildingId && buildings.length) setBuildingId(buildings[0].id); }, [buildings, buildingId]);
  useEffect(() => { if (buildingId) load(); }, [buildingId]); // eslint-disable-line react-hooks/exhaustive-deps

  async function load() {
    setLoading(true);
    const { data } = await supabase.from('service_contracts').select('*').eq('building_id', buildingId).order('service');
  ...
  function openEdit(r: ServiceContract) {
    ...
    setForm({
      ...
      billing_cycle: r.billing_cycle ?? 'monthly', notes: r.notes ?? '',
    });
    setOpen(true);
  }
  ...
  async function save() {
    ...
    setSaving(true);
    const attachment_url = file ? await uploadFile('attachments', `${buildingId}/contracts`, file) : null;
    ...
    const { error } = editId
      ...
      : await supabase.from('service_contracts').insert({ ...base, building_id: buildingId, created_by: profile?.id });
    setSaving(false);
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {canManage && buildingId && <Button onClick={openNew}><Plus size={16} /> {t('contracts.add')}</Button>}
        </div>
      </div>
              ...
              <Card key={r.id}>
                <CardBody>
                  ...
                  <div className="mt-3 pt-3 border-t border-slate-100 space-y-1 text-sm">
                    {r.amount_usd != null && (
                      <p className="text-slate-700 tnum font-medium">{money(Number(r.amount_usd))}
                        {r.billing_cycle && <span className="text-slate-400 text-xs font-normal"> / {t(`contracts.cycles.${r.billing_cycle}`)}</span>}
                      </p>
                    )}
                    ...
                    {r.attachment_url && (
                      <a href={r.attachment_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-indigo-600 hover:underline pt-1"><FileText size={12} /> {t('contracts.viewDoc')}</a>
                    )}
                  </div>
                </CardBody>
              </Card>
