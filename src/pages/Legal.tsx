 ...
 *
 * Exists because GeoNames is CC BY 4.0: free to use, but attribution is a
 * CONDITION of the licence, not a courtesy. The Lebanese place list ships
 * inside the product (3,568 places, generated by scripts/gen-cities.mjs), so
 * the credit has to be somewhere a user can actually see it. A source comment
 * does not satisfy the licence.
 *
 * Anything else that arrives under an attribution licence goes here too, which
 * is why it is a page rather than a line in the footer.
 */
export function Credits() {
  const { i18n } = useTranslation();
  const ar = i18n.language === 'ar';
  const fr = i18n.language === 'fr';
  const title = ar ? 'شكر وتراخيص' : fr ? 'Crédits et licences' : 'Credits & licences';
  return (
    <LegalShell title={title}>
      <h2>{ar ? 'بيانات الأماكن' : fr ? 'Données géographiques' : 'Place data'}</h2>
      {ar ? (
        <p>
          قائمة المدن والمناطق اللبنانية في أبنية مأخوذة من{' '}
          <a href="https://www.geonames.org/" target="_blank" rel="noopener noreferrer">GeoNames</a>,
          وهي متاحة بموجب رخصة{' '}
          <a href="https://creativecommons.org/licenses/by/4.0/" target="_blank" rel="noopener noreferrer">
            Creative Commons Attribution 4.0
          </a>. أُعيد ترتيب البيانات ودُمجت التهجئات المكرّرة، ولم تُعدَّل أسماء الأماكن نفسها.
        </p>
      ) : fr ? (
        <p>
          La liste des villes et quartiers libanais d'Abniyah provient de{' '}
          <a href="https://www.geonames.org/" target="_blank" rel="noopener noreferrer">GeoNames</a>,
          mise à disposition sous licence{' '}
          <a href="https://creativecommons.org/licenses/by/4.0/" target="_blank" rel="noopener noreferrer">
            Creative Commons Attribution 4.0
          </a>. Les données ont été reformatées et les orthographes en double fusionnées ; les noms de lieux
          eux-mêmes n'ont pas été modifiés.
        </p>
      ) : (
        <p>
          Abniyah's list of Lebanese cities and areas comes from{' '}
          <a href="https://www.geonames.org/" target="_blank" rel="noopener noreferrer">GeoNames</a>,
          made available under the{' '}
          <a href="https://creativecommons.org/licenses/by/4.0/" target="_blank" rel="noopener noreferrer">
            Creative Commons Attribution 4.0 International licence
          </a>. The data has been reformatted and duplicate spellings of the same place merged; the place
          names themselves are unchanged.
        </p>
      )}

      <h2>{ar ? 'التواصل' : fr ? 'Contact' : 'Contact'}</h2>
      <p>
        Tatawwor · Qoraytem, Beirut, Lebanon ·
        <a href="mailto:info@tatawwor.com"> info@tatawwor.com</a>
      </p>
    </LegalShell>
  );
}
