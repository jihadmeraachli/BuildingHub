...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function fetchProfile(userId: string) {
    ...
    // no-ops for strays). The RPC only flips status, so patch it locally
    // instead of re-fetching. NOTE: this trigger point is web-client-only -
    // a future non-web surface needs its own call (or a DB-side hook).
    if (p?.status === 'pending') {
      supabase.rpc('activate_invited_account').then(({ data }) => {
        if (data) setProfile({ ...p, status: 'active' });
      });
    }
  }
